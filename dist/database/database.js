...
const dbDir = path_1.default.resolve(__dirname, '../../db');
if (!fs_1.default.existsSync(dbDir)) {
    fs_1.default.mkdirSync(dbDir, { recursive: true });
}
const dbPath = path_1.default.resolve(dbDir, 'petshop.db');
...
async function seedDatabase() {
    ...
    for (const category of categories) {
        await db.run('INSERT INTO categories (name, description, image_url) VALUES (?, ?, ?)', [category.name, category.description, category.image_url]);
    }
    ...
    for (const product of products) {
        const result = await db.run('INSERT INTO products (name, description, price, stock, category_id, main_image_url) VALUES (?, ?, ?, ?, ?, ?)', [product.name, product.description, product.price, product.stock, product.category_id, product.main_image_url]);
        const productId = result.lastID;
        await db.run('INSERT INTO product_images (product_id, image_url, is_main) VALUES (?, ?, 1)', [productId, product.main_image_url]);
        await db.run('INSERT INTO product_images (product_id, image_url, is_main) VALUES (?, ?, 0)', [productId, `/images/products/additional/${product.name.toLowerCase().replace(' ', '-')}-1.jpg`]);
        await db.run('INSERT INTO product_images (product_id, image_url, is_main) VALUES (?, ?, 0)', [productId, `/images/products/additional/${product.name.toLowerCase().replace(' ', '-')}-2.jpg`]);
    }
